...
1. A Single Unified Architecture: The ultimate model is defined by the system architecture ([Human](human.md) + [Journal](journal.md) + [AI](ai.md) + [Time](time.md)). This architecture is the "operational core of the [Third Intelligence](third-intelligence.md) system" and is the subject of the final research question. The overall mechanism is described by the structural equation: [P(α)](p-alpha-probability-of-growth.md) = (S\_e × S\_t) / \|D-C\|.   
...
3. Core Mechanism Alignment: The sources consistently identify one central psychic process underlying all layers: Bion's [Alpha-Function](alpha-function.md).   
    - Psychological Core: The [Alpha-Function](alpha-function.md) is the fundamental psychic metabolism that turns the raw chaos of Beta-elements (unthinkable emotional material) into meaningful Alpha-elements (thinkable thoughts).   
